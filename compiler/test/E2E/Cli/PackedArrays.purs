-- | CLI-driven e2e (ADR 0031 phase 5) of the packed numeric arrays: build / set / index / length
-- | over `Wasm.I32Array` and `Wasm.F64Array`, plus the zero-initialised-lane contract that
-- | distinguishes their `unsafeNew` from `Wasm.Array.unsafeNew` (a fresh lane reads `0`, it does
-- | not trap). Built standalone by the real `purs-wasm build`.
...
spec :: Spec Unit
spec =
  describe "Packed numeric arrays (e2e/cli): Wasm.I32Array / Wasm.F64Array"
